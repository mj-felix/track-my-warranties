...
module.exports.isOwner = async (req, res, next) => {
  ...
  if (!entry || !entry.user.equals(req.user._id)) {
    if (req.headers.accept.indexOf("json") > -1) {
      return res
        .status(403)
        .json({ result: "You do not have permission to do that!" });
    } else {
...
module.exports.fileBelongsToUser = async (req, res, next) => {
  const { fileKey } = req.params;
  const entry = await Entry.findOne({ "files.storedFileName": fileKey });
  if (!entry || !entry.user.equals(req.user._id)) {
...
module.exports.isSpaceAvailable = async (req, res, next) => {
  ...
  if (storage > size500mb) {
    return res
      .status(418)
      .json({ result: "You have exceeded storage capacity!" });
  }
  next();
};
