const app = {
  ...
  file: {
    controler: {
      uploadFile: function (event) {
        ...
        axios
          .post(document.querySelector("#fileForm").action, formData, {
            headers: {
    ...
    view: {
      uploadedFile: function (res) {
        if (typeof res.data.file !== "undefined") {
          ...
          document
            .querySelector("#files a.deleteFile")
            .addEventListener("click", app.file.controler.deleteFile);
  ...
  utils: {
    security: {
      extractCsrfTokenFromHtmlMetadata() {
        return document
    ...
    view: {
      ...
      deleteWarrantyConfirmation: function (e) {
        if (confirm("Do you want to delete this warranty?")) {
          return true;
        } else {
          e.preventDefault();
          return false;
